...
-- One byte overhead for signaling this all. Need to add two with anything more.
flags = {
	IS_EPOE = 2 ^ 0,
	IS_ERROR = 2 ^ 1,
	IS_PRINT = 2 ^ 2,
...
function NewLine(flags)
	if HasFlag(flags, IS_SEQ) or HasFlag(flags, IS_MSG) or HasFlag(flags, IS_MSGC) or HasFlag(flags, IS_EPOE) then return "" end --	or HasFlag(flags,IS_ERROR)
...
function IsColor(val)
	...
	g = val.g
	b = val.b
	if not r or type(r) ~= "number" or not g or type(g) ~= "number" or not b or type(b) ~= "number" then return false end
