...
local function add(str, k)
	...
	elseif strtype == '^' then
		...
		if not ok then
			ErrorNoHalt"EPOE Regex parse failure: "
			e.internalPrint("Filters: Line " .. k .. ": Error in regex:" .. tostring(err))

			return false
		end

		table.insert(regex, {data, count})
	elseif strtype == '#' or strtype == '-' or strtype == '/' then
	else -- regex -- comment
...
local function ADD(X, Y)
	concommand.Add("epoe_filter_" .. X, function(_, cmd, _, filter)
		...
		if x ~= " " then
			Msg"[EPOE] "
			print"Syntax: epoe_filter_* N PARAMSTR (N is skip extra messages count)"
...
hook.Add(Tag, Tag, function(txt, flags)
	if not epoe_filtering:GetBool() or not e.filters.hasany or skipshit then return end
