...
function ProcessMessage(flags, str, col)
	...
	-- Store long messages
	if HasFlag(flags, IS_SEQ) then
		Buffer = Buffer .. str

		return
	elseif #Buffer > 0 then
		-- Data in buffer and no SEQ flag. We have something to print! TODO: See above todo.
		str = Buffer .. str
		Buffer = ''
	end
	...
	if not isEpoe and hook.Call(Should_TagHuman, nil, str, flags) == false then return end
...
function AddSub()
	if subscribed then
		internalPrint("Already subscribed.")
...
function MODULE.setenv(env)
	...
	local fenv = G.setmetatable({}, {
		__index = G.setmetatable(t, {
			__index = env or G.getfenv(2) or G
		})
	})
